...
def generate_one_episode_data(env: RlbenchEnv):
    ...
    #     this_data_collected['joint_action'], axis=0
    # )
    # # last delta action is zero i.e. no movement
    # this_data_collected['delta_joint_action'] = np.concatenate([
    #     this_data_collected['delta_joint_action'],
    #     np.zeros((1, this_data_collected['delta_joint_action'].shape[1]))
    # ])
    ...
    with mp.Pool(num_workers) as pool:
        args = [
            (
                ...
                task_indices[i*(len(task_indices)//num_workers):(i+1)*(len(task_indices)//num_workers)],
                sensors,
                data_dtype,
                save_subtasks
...
def uniprocess_core(
    ...
    env = RlbenchEnv(camera_names=sensors)
